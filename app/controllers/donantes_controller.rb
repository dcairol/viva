class DonantesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @donante.save
        format.html { redirect_to donantes_path, notice: "El donante #{donante.nombre} ha sido agregado." }
  ...
  def update
    respond_to do |format|
      if @donante.update(donante_params)
        format.html { redirect_to @donante, notice: "El donante #{donante.nombre} ha sido actualizado." }
  ...
  private
    ...
    def donante_params
      params.require(:donante).permit(:nombre, :telefono, :empresa, :monto, :periocidad)
    end
end
